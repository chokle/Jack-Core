...
- **Shipped:** Graceful video-preview fallback — a `<video>` element `onError` (unsupported container/codec, e.g. `.mov`/`.avi`/`.mkv`/`.3gp`) now swaps the player for a Jack-styled "can't be previewed in your browser" message with conversion guidance and a "Download original" link, instead of the browser's raw error; analysis/transcript panels stay visible and `video.status` is never touched by a playback failure. Upload-time Content-Type resolution also falls back to an extension-based lookup so a generic/missing browser-reported mimetype doesn't break playback for otherwise-supported files.
