...
- `artifacts/api-server/src/routes/interview.ts` — Interview Mode endpoints (start session, get, submit/skip answer, finish) plus the admin-gated `GET /interview/mentors` (mentor roster with session/answer counts), `POST /interview/mentors/:id/withdraw` (Mentor Withdrawal), and the admin-gated `POST /interview/answers/:id/redistill` (re-run distillation + knowledge-write verification for one answer). Answer submission distills + verifies inline and stamps `distillation_status` on the answer row
