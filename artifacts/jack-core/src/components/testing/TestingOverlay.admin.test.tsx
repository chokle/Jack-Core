...
describe("TestingOverlay consent boundary", () => {
  ...
  it("does not cancel an active recording for review-only withdrawal", async () => {
    state.cachedSession = {
      id: "11111111-1111-4111-8111-111111111111",
      organizationId: "org",
      pilotId: "pilot",
    ...
    act(() => {
      window.dispatchEvent(
        new CustomEvent("jack:telemetry-withdrawn", {
          detail: { withdrawn: ["conversationReview"] },
        }),
      );
    });
    expect(recordingServiceCancelSpy).not.toHaveBeenCalled();
