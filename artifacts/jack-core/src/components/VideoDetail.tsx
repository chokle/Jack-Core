...
interface VideoDetailProps {
  videoId: string;
  onBack: () => void;
  /** Surface restored by onBack, including sources opened outside the Library. */
  originLabel?: string;
...
export function VideoDetail({
  ...
  const [activeTab, setActiveTab] = useState<"transcript" | "analysis">(
    "analysis",
  );
  ...
  const { data: relatedVideos } = useFetchRelatedVideos(videoId, {
    query: { enabled: !!videoId, queryKey: ["related", videoId] },
  });
  ...
  const deleteMutation = useDeleteVideo({
    ...
    mutation: {
      onSuccess: () => {
        ...
        void queryClient.invalidateQueries({
          queryKey: getGetRecentVideosQueryKey(),
        });
        ...
        toast({
          ...
          description:
            err instanceof Error
              ? err.message
              : "Only your own duplicate uploads can be removed.",
          variant: "destructive",
        });
      },
    },
  ...
  const hasUploader =
    Boolean(
      (video as { uploaderUserId?: string | null } | undefined)?.uploaderUserId,
    ) ||
    Boolean(
      (video as { uploader_user_id?: string | null } | undefined)
        ?.uploader_user_id,
    );
  ...
  const uploaderUserId =
    duplicateDelete.uploaderUserId ?? duplicateDelete.uploader_user_id ?? null;
  const canRemoveOwnVideo =
    !isAdmin && Boolean(me?.userId) && uploaderUserId === me?.userId;
  ...
  const processingSteps = [
    {
      id: "transcribing",
      label: "Reading the footage",
      active: status === "transcribing",
      done: Boolean(video?.transcript),
    },
    {
      id: "analyzing",
      label: "Finding lessons and risks",
      active: status === "analyzing",
      done: Boolean(video?.analysis),
    },
    {
      id: "indexing",
      label: "Linking it into memory",
    ...
    try {
      ...
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: getGetVideoQueryKey(video.id),
        }),
      ...
      toast({
        ...
        description:
          "This video now sits under your node in Jack's memory graph.",
      });
    } catch (err) {
      toast({
        title: "Could not attach contributor",
        description:
          err instanceof Error ? err.message : "Try again after refreshing.",
        variant: "destructive",
      });
    } finally {
  ...
  if (isLoading || (video && video.id !== videoId)) {
    return (
      <div
        className="flex-1 flex flex-col h-full bg-background p-6"
        data-jack-surface="Video"
        data-jack-path={JSON.stringify([originLabel])}
      >
    ...
    return (
      ...
      >
        <div className="flex-none p-4 border-b border-border flex items-center bg-card/50 backdrop-blur">
          ...
          >
            <ArrowLeft className="h-4 w-4 mr-2" /> Back to {originLabel}
          </Button>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center text-center p-6 gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1 max-w-sm mx-auto">
              It may have been removed, or it's still finishing setup. Go back
              and try selecting it again.
            </p>
          </div>
          <Button size="sm" onClick={onBack} data-jack-action="back">
            <ArrowLeft className="h-4 w-4 mr-2" /> Back to {originLabel}
          </Button>
        </div>
      </motion.div>
    ...
    >
      <div className="flex-none p-4 border-b border-border flex items-center justify-between bg-card/50 backdrop-blur">
        ...
        <div className="flex items-center gap-2">
          {(video.status === "queued" ||
            ...
            <Button
              size="sm"
              onClick={() => transcribeMutation.mutate({ id: video.id })}
            >
              {video.status === "failed"
                ? "Retry Processing"
                : "Run Transcription"}
          ...
          <Button
            size="sm"
            variant="secondary"
            onClick={() => onOpenChat(`Based on the video "${video.title}"...`)}
          >
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-muted-foreground p-6 gap-3">
                ...
                <div>
                  <p className="font-semibold text-foreground">
                    Jack analyzed this video, but this file format can't be
                    previewed in your browser.
                  </p>
                  <p className="text-sm text-muted-foreground mt-1 max-w-sm mx-auto">
                    The transcript and analysis below are unaffected. For
                    in-browser playback, convert the source to MP4 (H.264 video
                    / AAC audio) and re-upload.
                  </p>
                </div>
            ...
            {video.status === "failed" && (
              <div className="mt-3 rounded-lg border border-destructive/30 bg-destructive/10 p-3">
                <p className="font-mono text-xs text-destructive">
                  Processing failed
                  {video.lastError ? `: ${video.lastError}` : "."}
                </p>
                <p className="font-mono text-xs text-muted-foreground mt-1">
                  Use "Retry Processing" above to try again
                  {canDeleteVideo
                    ? ", or remove this entry from the Library."
                    : "."}
                </p>
              </div>
            )}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-muted-foreground">
                            Keep this open to watch the analysis take shape, or
                            come back later.
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground border border-dashed border-border rounded-lg">
                      ...
                      {video.status === "completed" && (
                        <Button
                          ...
                          onClick={() =>
                            analyzeMutation.mutate({ id: video.id })
                          }
                  ...
                  {video.keyPoints && video.keyPoints.length > 0 && (
                    <div className="space-y-3">
                      <h3 className="font-semibold flex items-center text-sm">
                        <ListChecks className="h-4 w-4 mr-2 text-primary" /> Key
                        Takeaways
                      </h3>
      ...
      <AlertDialog open={confirmDeleteOpen} onOpenChange={setConfirmDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {isAdmin ? "Delete this video?" : "Remove your video?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {isAdmin
