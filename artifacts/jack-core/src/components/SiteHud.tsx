...
export function SiteHud({
  ...
  return (
    ...
    >
      <div className="site-hud__persistent" aria-label="Persistent site status">
        <span className="site-hud__identity">
          <Radio size={16} aria-hidden="true" /> SITE HUD{" "}
          <span className="site-hud__demo">SIMULATION</span>
        </span>
        <span className="site-hud__compact-state">
          <LinkIcon size={14} aria-hidden="true" /> {connectivity.mode} ·{" "}
        ...
        {onCloseDemo && (
          <button
            type="button"
            onClick={onCloseDemo}
            aria-label="Close demo site"
          >
            <X size={16} aria-hidden="true" />
          </button>
        )}
      </div>
      <div className="site-hud__page" hidden={!expanded}>
        <header className="site-hud__page-title">
          <div>
            <p>FIELD AWARENESS / DEMO SITE</p>
            <h1>Site radar</h1>
          </div>
          <span>Fictional contacts · Not for navigation</span>
        </header>
        <div className="site-hud__workspace">
          <div className="site-hud__instrument">
            <div className="site-hud__overview">
              <SiteRadar
                crew={plottedCrew}
                landmarks={view.landmarks.filter(
                  (landmark) => landmark.floorId === floor?.id,
                )}
                floorLabel={floor?.label ?? "No floor"}
                describeMember={describeMember}
                active={expanded}
              />
              <div className="site-hud__readout">
                <div
                  className="site-hud__mode"
                  role="status"
            ...
            >
              ...
              {panel === "floors" && (
                <>
                  ...
                  <p className="site-hud__help">
                    Radar shows scoped crew with a retained position on the
                    selected floor. Lost signals have no plotted position.
                  </p>
                </>
              )}
              {panel === "crew" && (
                <>
                  <p className="site-hud__help">
                    Only explicitly scoped demo contractors are named. Other
                    fresh nearby samples appear as a count.
                  </p>
                  ...
                  <p className="site-hud__help">
                    ...
                    forwarded sample. LAST KNOWN: stale or cloud unavailable.
                    SIGNAL LOST: no usable position. All are simulated.
                  </p>
                </>
              )}
              {panel === "safety" && (
                <>
                  <p className="site-hud__help">
                    Illustrative plan markers only. Follow actual site signage
                    and your site safety procedures.
                  </p>
                  <ul className="site-hud__list">
              ...
              {panel === "alerts" && (
                <>
                  <p className="site-hud__help">
                    Demo data-quality notices. This is not an emergency
                    monitoring service.
                  </p>
                  <ul className="site-hud__list">
              ...
              {panel === "signal" && (
                <>
                  ...
                  <p className="site-hud__help">
                    No radio scan, peer discovery, or real sync. Direct and
                    relayed links are fixture samples. Observations age without
                    refreshing.
                  </p>
                  <p className="site-hud__anonymous">
                    Demo queue:{" "}
                    <strong>{connectivity.pendingEvents.length}</strong> pending
                  ...
                  <div className="site-hud__demo-controls">
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (!browserIsOnline() || simulatedOffline.current)
                          return;
                        applyDemoEvent({
                          type: "sync-completed",
                  ...
                  {!browserOnline && (
                    <p className="site-hud__help">
                      Browser is offline. Reconnect is unavailable until the
                      browser returns online.
                    </p>
                  )}
                  {connectivity.mode === "OTG SYNCING" && (
                    <p className="site-hud__help">
                      Awaiting explicit demo reconciliation. No successful
                      server sync is implied.
                    </p>
                  )}
                  <p className="site-hud__help">
                    Demo events stay in memory for this tab and are discarded
                    when the demo closes. Reconciliation transmits nothing.
                  </p>
                </>
              )}
