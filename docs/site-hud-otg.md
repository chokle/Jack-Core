...
North and the fictional world rotate opposite the selected heading while the viewer marker stays fixed. Mobile layouts (under 768px, or coarse-pointer screens up to 1024px) always select device compass and omit manual controls. Desktop retains both modes. Device compass starts automatically where browser permissions allow; **Use device compass** requests access where a user gesture is required and allows retry after denial. Relative, invalid, excessively tilted, inaccurate or expired readings never become a valid heading: mobile shows an unavailable heading, while desktop retains manual fallback. Leaving the radar page removes sensor listeners and stops animation. Each four-second revolution pulses at every quarter turn, fades for 400ms, and stays dark for the remaining 600ms of that quarter. Pulse opacity and rotation use the same animation frame; reduced motion omits the sweep. Device readings are a magnetic/absolute reference, not verified true north. They remain local and are not stored or transmitted. Physical-device acceptance remains required.
