...
- **Confidence tradeoff:** the edge's scalar `meta.confidence` (a max over the mentor's answers, with no per-answer ledger) is deliberately NOT lowered on a per-answer withdrawal — a kept edge's node confidence can stay marginally overstated only when the removed answer held the max and others remain. Aliases + `meta.mergedFrom` are also deliberately LEFT intact (unattributed alternate wordings, not mentor data).
