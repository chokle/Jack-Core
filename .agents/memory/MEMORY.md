...
- [Blue crosshair canvas artifact](canvas-blue-crosshair.md) — intermittent full-viewport blue line = finite-but-degenerate gradient axis spike (non-finite throws, doesn't spike); if it recurs, bisect by unmounting KnowledgeGraph, don't add inert guards.
...
- [Reduced-motion desktop regression](reduced-motion-desktop.md) — "animates on mobile not desktop" across canvas anims = OS reduce-motion; headless/preview Chromium defaults to reduce; gate ambient motion via its own switch.
